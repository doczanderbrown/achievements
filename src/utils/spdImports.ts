...
const ACCOUNTABLE_ROLE_BOUNDARY_PATTERN =
  /(^|[\n;|•/]|\s)((?:decontamination|decon|sink|assembly|assemble|sterilization|sterilizer|sterilize|scan(?:\s|-)?to(?:\s|-)?or|scantoor|delivery|deliver|or(?:\s|-)?staff|surgery))\s*:/gim
...
const splitCommaDelimitedNames = (value: string) => {
...
const splitAccountableNames = (value: string): string[] =>
  value
    .replace(/\r/g, '\n')
    .split(/\n+|[;|•]+/)
    .flatMap((chunk) => {
      const normalizedChunk = normalizeWhitespace(chunk).replace(/^[,;/|•]+|[,;/|•]+$/g, '')
      if (!normalizedChunk) return []

      const explicitSegments = normalizedChunk
        .split(/\s+(?:&|and|\+)\s+|\s+\/\s+/i)
      ...
      return splitCommaDelimitedNames(normalizedChunk)
    })
    .map((name) => normalizeWhitespace(name).replace(/^[,;/|•]+|[,;/|•]+$/g, ''))
    .filter(Boolean)

const splitAccountableSegments = (value: string) => {
  const entries: Array<{ roleLabel: string | null; value: string }> = []
  const matches = [...value.matchAll(ACCOUNTABLE_ROLE_BOUNDARY_PATTERN)].map((match) => ({
    labelStart: (match.index ?? 0) + String(match[1] ?? '').length,
  }))

  if (!matches.length) {
    return [{ roleLabel: null, value }]
  }

  if (matches[0].labelStart > 0) {
    const leadingValue = value.slice(0, matches[0].labelStart).trim()
    if (leadingValue) {
      entries.push({ roleLabel: null, value: leadingValue })
    }
  }

  for (let index = 0; index < matches.length; index += 1) {
    const start = matches[index].labelStart
    ...
    if (!match) {
      entries.push({ roleLabel: null, value: segment })
      continue
    }
...
const parseAccountableEntries = (value: unknown): ParsedAccountableEntry[] => {
  const text = String(value ?? '').trim()
  if (!text) return []

  // Quality exports are inconsistent: support both one-entry-per-line cells and
  // inline cells that concatenate multiple role assignments or multiple names.
  return splitAccountableSegments(text).flatMap((segment) =>
    splitAccountableNames(segment.value).map((name) => ({
...
const calculateDefectRate = (
  ...
  const opportunity = buildQualityOpportunities(row).assembly
  const incidents = stats.incidents.assembly
  if (incidents === 0 && opportunity === 0) return 0

  return clampRate(incidents / Math.max(opportunity, 1))
}
